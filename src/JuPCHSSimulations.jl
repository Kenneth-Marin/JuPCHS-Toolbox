...
function explicit_euler(PCHS::typePCHS ,u::Function,xini::Vector,nt::Int64)    
    ...
    for k = 2:nt
        ut = u(x,(k-1)*PCHS.dt)
        f = (PCHS.J(x,ut)-PCHS.R(x))*PCHS.dH(x)+PCHS.G(x)*ut
        x = x + PCHS.dt*f
        xode[:,k] = x
        uode[:,k] = ut
...
function implicit_euler(PCHS::typePCHS ,u::Function,xini::Vector,nt::Int64)   
    ...
    for k = 2:nt
        xa = x
        ut = u(x,(k-1)*PCHS.dt)
        f = (PCHS.J(xa,ut)-PCHS.R(xa))*PCHS.dH(xa)+PCHS.G(xa)*ut
        xn = x + PCHS.dt*f
        err = norm(xn-xa)
        xa = xn
        m = 0
...
function implicit_midle_point(PCHS::typePCHS ,u::Function,xini::Vector,nt::Int64)   
    ...
    for k = 2:nt
        xm = x
        ut = u(xm,(k-1)*PCHS.dt)
        f = (PCHS.J(xm,ut)-PCHS.R(xm))*PCHS.dH(xm)+PCHS.G(xm)*ut
        xm = x + PCHS.dt*f/2
        err = norm(xm-x)        
        m = 0
        while (err>EPSILON)&(m<MAXITER)
...
function explicit_runge_kutta(PCHS::typePCHS ,u::Function,xini::Vector,nt::Int64)    
    ...
    yode[:,1] = transpose(PCHS.G(x))*PCHS.dH(x)
    res.H[1] = PCHS.H(x)    
    F(x,ur) = (PCHS.J(x,ur)-PCHS.R(x))*PCHS.dH(x)+PCHS.G(x)*ur
    for k = 2:nt
        ut = u(x,(k-1)*PCHS.dt)
        f1 = F(x,ut)
...
# Implicit RK (NO hay literatura de este metodo)
function implicit_runge_kutta(PCHS::typePCHS ,u::Function,xini::Vector,nt::Int64)
    res = DataFrame()    
    res.t = (1:nt)*PCHS.dt
    xode = zeros(PCHS.nx,nt)
...
function trapezoid_rule(PCHS::typePCHS,u::Function,xini::Vector,nt::Int64)   
    res = DataFrame()    
    res.t = (1:nt)*PCHS.dt
    xode = zeros(PCHS.nx,nt)
    uode = zeros(PCHS.nu,nt)
    ...
    for k = 2:nt
        xa = x
        ut = u(x,(k-1)*PCHS.dt)
        f_ = (PCHS.J(x,ut)-PCHS.R(x))*PCHS.dH(x)+PCHS.G(x)*ut
        f = (PCHS.J(xa,ut)-PCHS.R(xa))*PCHS.dH(xa)+PCHS.G(xa)*ut
        xn = x + PCHS.dt/2*(f_ + f)
        err = norm(xn-xa)
        xa = xn
...
function backward_diff(PCHS::typePCHS,u::Function,xini::Vector,nt::Int64)   
    res = DataFrame()    
    res.t = (1:nt)*PCHS.dt
    xode = zeros(PCHS.nx,nt)
    uode = zeros(PCHS.nu,nt)
    ...
    for k = 2:nt
        ...
        ut = u(x,(k-1)*PCHS.dt)
        f = (PCHS.J(xa,ut)-PCHS.R(xa))*PCHS.dH(xa)+PCHS.G(xa)*ut
        xn = 18/11*x - 9/11*xk2 + 2/11*xk3 + PCHS.dt*(6/11)*f
        err = norm(xn-xa)
        xa = xn
        m = 0
        while (err>EPSILON)&(m<MAXITER)
...
function hamilton_discret(PCHS::typePCHS,u::Function,xini::Vector,nt::Int64)   
    res = DataFrame()    
    res.t = (1:nt)*PCHS.dt
    xode = zeros(PCHS.nx,nt)
    uode = zeros(PCHS.nu,nt)
    ...
    for k = 2:nt
        xa = x
        ut = u(x,(k-1)*PCHS.dt)
        f = (PCHS.J(x,ut)-PCHS.R(x))*PCHS.dH(xa)+PCHS.G(x)*ut
        xn = x + PCHS.dt*f
        err = norm(xn-xa)
        xa = xn
        m = 0
...
function Simulate(;PCHS=nothing,u=nothing,xini=nothing,nt=nothing,method=nothing)
    ...
    elseif method in ["MidPoint","ImplicitMidPoint","iMP"]
        res = implicit_midle_point(PCHS,um,xini,nt)       
    elseif method in ["RungeKutta","RK"]
        res = explicit_runge_kutta(PCHS,um,xini,nt)       
    elseif method in ["ImplicitRungeKutta","iRK"]
        res = implicit_runge_kutta(PCHS,um,xini,nt)
    elseif method in ["trapezoid","Trapezoid","Trapezoidal"]
        res = trapezoid_rule(PCHS,um,xini,nt)
    elseif method in ["BackwardDiff","BackwardDiff","BackwardDifferentiation"]
        res = backward_diff(PCHS,um,xini,nt)
    elseif method in ["Hamilton","Hamiltonian","H"]
        res = hamilton_discret(PCHS,um,xini,nt)
    elseif method in ["Euler","ForwardEuler"]
        res = explicit_euler(PCHS,um,xini,nt)    
    else
        print_warning("Unrecognized method. Defaulting to Euler")
        res = explicit_euler(PCHS,um,xini,nt) 
    end 
    return res
end